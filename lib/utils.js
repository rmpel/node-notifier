...
module.exports.fileCommandJson = function (notifier, options, cb) {
  ...
  return cp.execFile(notifier, options, function (error, stdout, stderr) {
    // terminal-notifier >= 3 exits with 6 when -timeout expires. That is a
    // regular outcome for node-notifier, not an error.
    if (error && error.code === MAC_EXIT_NO_RESPONSE) {
      error = null;
    }
    if (error) return cb(error, stdout);
    ...
    try {
      const data = parseMacResponse(options, stdout);
...
// Exit codes of terminal-notifier >= 3.
const MAC_EXIT_NOT_AUTHORIZED = 3;
const MAC_EXIT_NO_RESPONSE = 6;
module.exports.MAC_EXIT_NOT_AUTHORIZED = MAC_EXIT_NOT_AUTHORIZED;
module.exports.MAC_EXIT_NO_RESPONSE = MAC_EXIT_NO_RESPONSE;

/**
 * Normalises what terminal-notifier prints on stdout into the object shape
 * node-notifier has always handed to callbacks and event listeners:
 * `{ activationType, activationValue }`.
 *
 * terminal-notifier 1.x/2.x (and forks used through `customPath`) print JSON
 * when passed `-json`. terminal-notifier >= 3 prints a single line: the
 * clicked action title, the typed reply, or one of the markers `@TIMEOUT`,
 * `@CLOSED`, `@ACTIONCLICKED`. `-list` and `-remove` print free text.
 */
function parseMacResponse(argsList, stdout) {
  argsList = argsList || [];
  try {
    return JSON.parse(stdout);
  } catch (e) {
    // Not JSON; fall through to the plain-text protocol of terminal-notifier 3.
  }

  if (inArray(argsList, '-list') || inArray(argsList, '-remove')) {
    return stdout;
  }
  ...
  switch (text) {
    case '@TIMEOUT':
      return { activationType: 'timedOut' };
    case '@CLOSED':
  ...
  if (inArray(argsList, '-reply')) {
    return { activationType: 'replied', activationValue: text };
  }
  if (inArray(argsList, '-action')) {
    return { activationType: 'actionClicked', activationValue: text };
  }
  return stdout;
}
module.exports.parseMacResponse = parseMacResponse;

/**
 * UNUserNotificationCenter (terminal-notifier >= 3) refuses to ask for
 * notification permission for an app bundle LaunchServices has never seen,
 * failing with "Notifications are not allowed for this application". A bundle
 * that lives inside node_modules is exactly that, so register it once.
 */
const LSREGISTER =
  '/System/Library/Frameworks/CoreServices.framework/Frameworks/' +
  'LaunchServices.framework/Support/lsregister';

module.exports.isMacNotAuthorizedError = function (error, stderr) {
  return (
    !!error &&
    error.code === MAC_EXIT_NOT_AUTHORIZED &&
    /not allowed for this application/i.test(String(stderr || error.message))
  );
};

module.exports.registerMacNotifier = function (notifier, cb) {
  const match = /^(.*\.app)\/Contents\/MacOS\//.exec(notifier);
  if (!match) return cb(new Error('Not an app bundle: ' + notifier));
  cp.execFile(LSREGISTER, ['-f', match[1]], function (error) {
    cb(error || null);
  });
};
...
module.exports.mapToMac = function (options) {
  ...
  // terminal-notifier >= 3 has no way to set a custom icon or sender: the
  // UserNotifications framework always uses the sending bundle's identity.
  // Both flags are accepted but only produce warnings on stderr, so drop them.
  delete options.appIcon;
...
module.exports.isMojaveOrLater = function () {
  return (
    os.type() === 'Darwin' &&
    semver.satisfies(garanteeSemverFormat(os.release()), '>=18.0.0')
  );
};

// Deprecated alias, kept so existing code that stubs it keeps working.
module.exports.isMountainLion = module.exports.isMojaveOrLater;
